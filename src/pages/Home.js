...
const Home = () => {
  ...
  const [sumHistory, setSumHistory] = useState("Valores");

  const handleClick = (e, operator) => {
    const tempSumHistory = sumHistory.replace("Valores", "");

    if (operator === "=") {
      calculate();
    } else if (operator === "C") {
      reset();
    } else if (operator === "erase") {
      backspace();
    } else {
  ...
  return (
    <IonPage>
      <IonHeader>Calculadora Guillermo</IonHeader>
